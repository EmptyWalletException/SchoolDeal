...
4:增加了注册账号的功能,在注册账号时会同时注册本地账号和用户信息以及绑定普通用户的角色,使用了mybatis的insert返回主键的功能,注意此功能在Mapper里配置完之后还需要在方法里使用实体类的get方法才能取回主键值,insert语句返回的只能是受影响的行数;明天完善各个功能模块中的输入框校验及后端通过组件完成校验的功能;
