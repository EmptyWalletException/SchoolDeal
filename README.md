# SchoolDeal
使用SSM框架开发的一个简单的校园多商家食堂点餐平台

2018/6/11

1:在顶部导航栏上增加了用户头像框和用户信息操作,当用户未登录时显示的是登录和注册链接,登录后显示个人中心和退出链接;
项目还没做完善,剩下卖家部分的功能没有实现,ssm框架目前就做到这里,接下来将此项目改造成springboot框架https://github.com/EmptyWalletException/DealSites.git,
并使用springboot框架实现其它强大的功能组件;
...
4:已经将项目的级联部分修改完成,同时修复了在某店铺内点击分类时查出的是该分类所有店铺商品的逻辑bug,目前发现的问题是使用json传递查询出来的数据时不能开启懒加载,否则会报一个json序列化异常;
